...
import { Header, SignIn, Footer } from "../components";
import { UserContext } from "../context/";
import getOptions from "../util/getOptions";

const SignInPage = ({ options }) => {
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem("auth-token").length > 0 && callback) {
    ...
    if (!user.authToken) {
        return (
            <>
                <Head>
                    <title>Sign In - {options["site-title"]}</title>
                </Head>
